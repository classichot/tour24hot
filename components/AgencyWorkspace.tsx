...
import { isInbound, statusInfo } from "@/lib/helpers";
...
export default function AgencyWorkspace({ inbound }: { inbound: boolean }) {
  ...
  const pkgRows = DATA.packages.filter((p) => p.agency === "siam" && isInbound(p) === inbound);
  const bookingRows = DATA.agencyBookings.filter((b) => {
    const pkg = pkgById(b.pkg);
    return pkg && isInbound(pkg) === inbound;
  });
  const topRows = an.top.filter((x) => {
    const pkg = pkgById(x.pkg);
    return pkg && isInbound(pkg) === inbound;
  });
  const priceRows = an.pricePosition.filter((x) => {
    const pkg = pkgById(x.pkg);
    return pkg && isInbound(pkg) === inbound;
  });
  const demandRows = inbound
    ? DEMAND.filter((d) => /thailand|inbound/i.test(d.destination))
    : DEMAND.filter((d) => d.destination === "Japan" || d.destination === "China" || d.destination === "Taiwan");
  const viewsFor = (id: string) => (id === "jp01" ? "6,120" : id === "th01" ? "3,280" : "4,380");
  const cmpFor = (id: string) => (id === "jp01" ? "2,410" : id === "th01" ? "1,640" : "1,190");
  const sampleFile = inbound ? "CNX-CEI-GTR-5D4N-INBOUND.pdf" : "TOKYO-FUJI-5D3N-OCT26.pdf";
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-[22px] pb-20">
      <div className="pt-[22px] pb-3 flex items-end justify-between gap-4 flex-wrap border-b-2 border-divider">
        <div>
          <div className="kicker">{inbound ? t.navInAgency : L(me.name)}</div>
          <h1 className="mt-1 text-[clamp(24px,3vw,38px)]">{inbound ? t.agTitleInbound : t.agTitle}</h1>
            ...
            {upStage === 4 && fields.length > 0 && (
              <div className="border-2 border-text">
                <div className="px-3.5 py-3 border-b-2 border-divider flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <span className="tag tag-accent">{fileName || sampleFile}</span>
                </div>
          ...
          <div className="overflow-x-auto border-2 border-divider">
            <div className="min-w-[900px]">
              ...
              {bookingRows.length === 0 && (
                <div className="px-3.5 py-8 text-sm text-neutral-700">{t.noResultsSub}</div>
              )}
              {bookingRows.map((b) => {
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-[26px]">
            ...
            <div>
              ...
              <div className="flex flex-col gap-0.5 bg-divider border-2 border-divider">
                {(topRows.length ? topRows : pkgRows.slice(0, 3).map((p) => ({ pkg: p.id, views: 3280, bookings: 12 }))).map((x) => (
              ...
              <div className="flex flex-col gap-2">
                {(priceRows.length ? priceRows : pkgRows.slice(0, 2).map((p) => ({ pkg: p.id, delta: -3.1 }))).map((x) => (
