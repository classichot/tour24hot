...
export type TourCatalogMode = "outbound" | "inbound";
...
function locText(v: { th: string; en: string; zh?: string; ru?: string } | string) {
  return typeof v === "string" ? v : `${v.th} ${v.en} ${v.zh || ""} ${v.ru || ""}`;
}

function filtersFromSearch(params: URLSearchParams, mode: TourCatalogMode): Filters {
  ...
  if (mode === "inbound") {
    const regionMatch = DATA.inboundRegions.find((r) => {
      const needle = (region || q || "").toLowerCase();
      return needle && (r.q.toLowerCase() === needle || r.name.en.toLowerCase() === needle);
    });
    if (regionMatch) next.place = regionMatch.q;
    else if (q) next.q = q;
  } else {
    if (q) next.q = q;
    if (country && country !== "Thailand") next.place = country;
  }
...
export default function TourCatalog({ mode }: { mode?: TourCatalogMode }) {
  return (
    <Suspense fallback={<main className="max-w-[1400px] mx-auto px-[22px] pt-[22px]" />}>
      <TourCatalogInner mode={mode} />
    </Suspense>
  );
}

function TourCatalogInner({ mode }: { mode?: TourCatalogMode }) {
  const { t, L, money } = useApp();
  const params = useSearchParams();
  const resolved: TourCatalogMode =
    mode ?? (params.get("dir") === "inbound" || params.get("country") === "Thailand" ? "inbound" : "outbound");
  useInboundScope(resolved === "inbound");
  const inbound = resolved === "inbound";
  const [f, setF] = useState<Filters>(() => filtersFromSearch(params, resolved));
  const [sort, setSort] = useState("real");
  const [filtersOpen, setFiltersOpen] = useState(false);

  useEffect(() => {
    setF(filtersFromSearch(params, resolved));
  }, [params, resolved]);
  ...
  const placeOpts = useMemo(() => {
    if (inbound) {
      return [
        { v: "any", label: L({ th: "ทุกเส้นทาง", en: "All regions", zh: "全部线路" }) },
        ...DATA.inboundRegions.map((d) => ({ v: d.q, label: L(d.name) })),
      ];
    }
    return [
      { v: "any", label: L({ th: "ทุกประเทศ", en: "All countries", zh: "全部国家" }) },
      ...DATA.destinations
        .filter((d) => DATA.packages.some((p) => p.country.en === d.name.en && !isInbound(p)))
  ...
  const results = useMemo(() => {
    ...
    let out = DATA.packages.filter((p) => {
      if (isInbound(p) !== inbound) return false;
      if (q) {
      ...
      if (f.place !== "any") {
        if (inbound) {
          const hay = [locText(p.title), locText(p.city)].join(" ").toLowerCase();
          if (!hay.includes(f.place.toLowerCase())) return false;
        } else if (p.country.en !== f.place) {
          return false;
        }
      }
  ...
  if (f.place !== "any") {
    const c = placeOpts.find((o) => o.v === f.place);
    chips.push({ key: "place", label: c?.label || f.place, clear: () => set("place", "any") });
  }
  if (f.dur !== "any") {
    const durLabel =
      f.dur === "short"
  ...
  const wherePh = inbound
    ? L({ th: "เชียงใหม่ เชียงราย สามเหลี่ยมทองคำ…", en: "Chiang Mai, Chiang Rai, Golden Triangle…", zh: "清迈、清莱、金三角…" })
    : t.fWherePh;
    ...
    <main className="max-w-[1400px] mx-auto px-[22px] pb-[100px]">
      <div className="pt-5 pb-4 border-b-2 border-divider">
        <div className="flex items-end justify-between gap-4 flex-wrap">
          <div>
            <div className="kicker">{inbound ? t.navInbound : t.navSearch}</div>
        ...
        >
          ...
          <div className="p-3.5 flex flex-col gap-3.5 lg:max-h-[calc(100vh-168px)] lg:overflow-auto">
            <div className="field">
              ...
              <input className="input" type="text" value={f.q} placeholder={wherePh} onChange={(e) => set("q", e.target.value)} />
            </div>
            <div className="field">
              <label>{inbound ? t.fRegion : t.fCountry}</label>
              <select className="input" value={f.place} onChange={(e) => set("place", e.target.value)}>
                {placeOpts.map((o) => (
